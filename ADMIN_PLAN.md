...
- [x] Membuat migrasi database SQL untuk tabel `roles`, `permissions`, `role_permissions`, dan `audit_logs`.
- [x] Membuat middleware Go Chi `RequirePermission(code string)` untuk menggantikan *role check* ter-hardcode.
- [x] Mengimplementasikan *Audit Logger Interceptor* di backend Go.

### 📌 Tahap 2: Manajemen User & Link Global Oversight
- [x] Membangun UI Admin Management User (Suspend/Unsuspend, Revoke Session, Impersonate).
- [x] Membangun UI Global Link Inspector dengan fitur pembekuan link berbahaya (*Abuse Ban*).

### 📌 Tahap 3: Dynamic Menu Builder & Menu Matrix
- [x] Membuat tabel database `navigation_menus` dan API penyaji menu berdasarkan *Permission* pengguna.
- [x] Mengintegrasikan `AppSidebar` React SPA agar me-render menu secara dinamik dari backend API.
- [x] Membangun antarmuka UI *Menu Drag-and-Drop Builder* di Portal Admin.

### 📌 Tahap 4: System Configuration, Feature Flags & Monitoring Dashboard
- [x] Membangun modul *System Configuration & Feature Flags Toggle*.
- [x] Memintegrasikan pemantauan antrean *Asynq Worker* & *Audit Log Diff Explorer*.
