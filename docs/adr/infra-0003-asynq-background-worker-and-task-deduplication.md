...
2. **Multi-Target Containerization (`Dockerfile` with `target: worker`)**:
   - Containerized as `url-shortener-worker` in `compose.yml` using `dockerfile: Dockerfile` with target `worker` so API nodes and background workers share a unified build cache while auto-scaling independently.
...
- Consolidated single `Dockerfile` maintenance with shared layer caching and clean multi-target container deployment via `compose.yml`.
