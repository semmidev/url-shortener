...
import (
	...
	_ "github.com/semmidev/url-shortener/server/docs"
...
func BuildRouter(cfg config.Config, pool *pgxpool.Pool, appLogger *logger.Logger) (chi.Router, error) {
	...
	// CORS Middleware to handle browser preflight OPTIONS requests (e.g. from Swagger UI or Frontend)
	...
	// Public Swagger Documentation UI Endpoint
	...
	// Public Redirection Endpoint (GET /{code})
	r.Get("/{code}", redirectH.Redirect)

	// API v1 Routes (Strictly Authenticated)
